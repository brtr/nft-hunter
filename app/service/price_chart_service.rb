class PriceChartService
  ...
  def get_holding_data
    ...
    TargetNftOwnerHistory.holding.where(event_date: [start_date..end_date], nft_id: nft_id).group_by(&:event_date).each do |date, records|
      result[:data].merge!({date => records.map{|x| x.data[:data].merge!(date: date)}})
    end
    result
  end
  ...
  def get_purchase_data
    result = {labels: labels, data: {}}
    TargetNftOwnerHistory.purchase.where(event_date: [start_date..end_date], nft_id: nft_id).group_by(&:event_date).each do |date, records|
      result[:data].merge!({date => records.map{|x| x.data[:data].merge!(date: date)}})
    end
    result
  end
